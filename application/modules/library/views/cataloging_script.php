...
<script type="text/javascript" src="<?=load_asset()?>js/xlsx.core.min.js"></script>
<script type="text/javascript" src="<?=load_asset()?>js/Blob.js"></script>
<script type="text/javascript" src="<?=load_asset()?>js/FileSaver.js"></script>
<script type="text/javascript" src="<?=load_asset()?>js/Export2Excel.js"></script>
...
cataloging.viewTuitionFee = function(courseAnnualFeeID){
    ...
    $.post("<?=  api_url()?>c_cataloging/retrieveMaterial",{ID : courseAnnualFeeID},function(data){
        ...
        if(!response["error"].length){
            ...
            $("#catalogingDateAcquired").val(((response["data"]["date_acquired"]==0)?"":dt.getFullYear()+"-"+("0"+(dt.getMonth()+1)).slice(-2)+"-"+("0"+dt.getDate()).slice(-2)));
            $("#catalogingSupplier").val(response["data"]["supplier"]);
            $("#catalogingCostPrice").val(response["data"]["cost_price"]);
            cataloging.changeFormAction(1);
...
cataloging.retrieveList = function(filter){
    ...
    $("#btn-export-data").click(function (e) {
        ...
        b.button("loading");
        $.post("<?=  api_url()?>c_cataloging/retrieveForExcel",{},function(data){
            var response = JSON.parse(data);
            var table_default = "<table id='exportdummy'><thead><tr><td>Acc. No.</td><td>Date Received</td><td>Class No.</td><td>Author</td><td>Title</td><td>Place of Publication</td><td>Publisher</td><td>Copyright</td><td>Remarks</td></tr></thead><tbody>";
            var table_data = table_default;
            for(var x = 0; x < response["data"].length; x++){
                dt = new Date((response["data"][x]["date"])*1000);
                table_data += "<tr><td>"
                    +response["data"][x]["accession"]+"</td><td>"
                    +((response["data"][x]["date"]==0)?"":dt.getFullYear()+"-"+("0"+(dt.getMonth()+1)).slice(-2)+"-"+("0"+dt.getDate()).slice(-2))+"</td><td>"
                    +response["data"][x]["class"]+"</td><td>"
                    +response["data"][x]["author"]+"</td><td>"
                    +response["data"][x]["title"]+"</td><td>"
                    +response["data"][x]["place"]+"</td><td>"
                    +response["data"][x]["publisher"]+"</td><td>"
                    +response["data"][x]["copyright"]+"</td><td>"
                    +response["data"][x]["remarks"]+"</td></tr>";

            }
            $(".exportdummy").html(table_data + "</tbody></table>");
            export_table_to_excel("exportdummy");
