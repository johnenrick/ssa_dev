...
materialAccessControl.viewTuitionFee = function(courseAnnualFeeID){
    $("#materialAccessControlForm").trigger("reset");
    $.post("<?=  api_url()?>c_material_access_control/retrieveMaterialAccessControl",{ID : courseAnnualFeeID, byusers : $(".users").length},function(data){
        var response = JSON.parse(data);
        var dt;
...
materialAccessControl.removeList = function(row){
    $.post("<?=  api_url()?>c_material_access_control/deleteMaterialAccessControl",{ID : row.attr("assment_type_id"), libraryusers : $(".users").length},function(data){
        var response = JSON.parse(data);
        if(!response["error"].length){
            row.remove();
            materialAccessControl.retrieveList();
