...
$(document).ready(function(){
    ...
    $("#materialAccessControlCreateFormButton").click(function(){
        ...
        $("#materialAccessControlForm").attr("action", "<?=api_url()?>c_material_access_control/createMaterialAccessControl");
...
materialAccessControl.viewTuitionFee = function(courseAnnualFeeID){
    $("#materialAccessControlForm").trigger("reset");
    $.post("<?=  api_url()?>c_material_access_control/retrieveMaterialAccessControl",{ID : courseAnnualFeeID},function(data){
        var response = JSON.parse(data);
        var dt;
        console.log(response);
        if(!response["error"].length){
            $("#materialAccessControlForm").trigger("reset");
            $("#materialAccessControlID").val(response["data"]["ID"]*1);
            $("#materialAccessControlDescription").val(response["data"]["description"]);
            $("#materialAccessControlPeriodNumber").val(response["data"]["period"]);
            $('#materialAccessControlPeriodUnit option[value="'+response["data"]["unit"]+'"]').prop('selected', true);
            $("#materialAccessControlFineRate").val(response["data"]["fine_rate"]);
            materialAccessControl.changeFormAction(1);
            $("#materialAccessControlForm").attr("action", "<?=api_url()?>c_material_access_control/updateMaterialAccessControl");
            $("#materialAccessControlDiv").slideDown();
        }
    });
}
materialAccessControl.removeList = function(row){
    $.post("<?=  api_url()?>c_material_access_control/deleteMaterialAccessControl",{ID : row.attr("assment_type_id")},function(data){
        var response = JSON.parse(data);
        if(!response["error"].length){
            row.remove();
            materialAccessControl.retrieveList();
...
materialAccessControl.retrieveList = function(filter){
    ...
    $.post("<?=api_url()?>c_material_access_control/retrieveMaterialAccessControl", filter, function(data){
        var response = JSON.parse(data);
        console.log(response);
