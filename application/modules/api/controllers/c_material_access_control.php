 ...
 */
class c_material_access_control extends API_Controller{

    public function __construct() {
        parent::__construct();
        $this->load->model("m_material_access_control");
    ...
    public function createMaterialAccessControl(){
        ...
        if(!$this->checkACL(user_type(),1) && (API_Controller::STUDENT_MANAGEMENT)){ //if not admin
            ...
            $this->form_validation->set_rules('libraryuserID', 'Control Period', 'required');
            $this->form_validation->set_rules('periodNumber', 'Control Period', 'required');
            $this->form_validation->set_rules('periodUnit', 'Control Period', 'required');
            $this->form_validation->set_rules('fineRate', 'Control Fine Rate', 'required');
            if($this->form_validation->run()){
                $result = $this->m_material_access_control->createMaterialAccessControl(
                        $this->input->post("description"),
                        $this->input->post("libraryuserID"),
                        $this->input->post("periodNumber"),
                        $this->input->post("periodUnit"),
                        $this->input->post("fineRate")
                        );

                if($result){
                    $this->m_change_log->createChangeLog(4, user_id() , json_encode(array(
                        "message" => "Create Material Access Control",
